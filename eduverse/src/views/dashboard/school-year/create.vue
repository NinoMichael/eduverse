...
const handleSubmit = async () => {
	...
	const success = await schoolYearStore.createSchoolYear(data);
	...
	if (success) {
		...
		if (configureImmediately.value) {
			router.push("/dashboard/school-year/event/configure");
		} else {
			router.push("/dashboard/school-year/list");
		}
	} else {
		toast.error(
			schoolYearStore.error || "Erreur lors de la création de l'année scolaire"
		);
	}
};
</script>
...
<template>
	<div class="space-y-4 lg:space-y-6">
		...
		<div class="relative space-y-1 bg-primary text-white p-8 rounded-lg w-full">
			<h2 class="text-lg lg:text-xl font-semibold">Créer une année scolaire</h2>
			...
			>
				...
				<div class="flex flex-col sm:flex-row gap-4 w-full md:w-auto">
					...
					>
						...
						/>
						<font-awesome-icon v-else icon="plus-circle" class="mr-2" />
						<span>{{ form.isSubmitting.value ? "Création..." : "Créer" }}</span>
					</button>
				</div>
			</div>
		</form>
